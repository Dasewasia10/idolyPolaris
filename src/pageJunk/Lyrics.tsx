  ...
  return (
    <>
      <div className="h-max w-auto z-10 flex flex-col p-2 gap-2 mb-10 lg:mb-0 mt-10 lg:mt-0">
        <section id="leftConsole" className="absolute">
          ...
          >
            ...
            <button
              ...
              className="absolute -right-10 top-1/2 -translate-y-1/2 h-20 w-10 bg-slate-900 text-white rounded-r-xl border-y border-r border-slate-700 flex items-center justify-center shadow-lg"
            >
              {isLeftMenuOpen ? "✕" : "☰"}
            </button>
          </div>
        ...
        <section className="flex flex-col w-full mx-auto overflow-auto gap-4 scrollbar-minimal h-[60vh] lg:h-[calc(100vh-250px)] shadow-inner border rounded-lg bg-white/50 backdrop-blur-sm">
          ...
          {activeData.length > 0 && (
            <table className="table-auto w-full bg-white border-collapse">
              <thead className="sticky top-0 bg-slate-800 text-white z-20 shadow-md">
                <tr>
                  {activeColumns.kanji && (
                    <th className="px-3 py-3 text-sm font-semibold border-b">
                      Kanji
                    </th>
                  )}
                  ...
                  <React.Fragment key={item.id}>
                    {item.kanji.map((kanji: string, index: number) => (
                      <tr
                        ...
                        className="odd:bg-slate-50 even:bg-white hover:bg-blue-50 transition-colors duration-150"
                      >
                        {activeColumns.kanji && (
                          <td className="border-b border-slate-100 px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap min-w-[200px]">
                            {parseText(kanji)}
                          </td>
                        )}
                        {activeColumns.romaji && (
        ...
        <div
          id="belowConsole"
          className="w-full flex flex-col lg:flex-row items-stretch lg:items-center gap-4 mt-4 px-2"
        >
          ...
          {/* Toggle Controls */}
          <div className="flex flex-wrap items-center justify-center gap-2 p-2 bg-slate-100 rounded-xl shadow-sm border border-slate-200">
            {/* Gunakan grid atau flex-wrap untuk tombol toggle agar tidak melebar keluar layar di mobile */}
